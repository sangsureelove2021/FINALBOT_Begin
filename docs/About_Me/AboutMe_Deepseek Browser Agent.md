...
### 📝 บันทึกพิเศษสำหรับโปรเจกต์นี้ (วิธีเรียกใช้งาน DSA ฉบับสมบูรณ์)
เพื่อให้การเรียกใช้ `dsa` (DeepSeek Agent) ทำงานได้อย่างเสถียรที่สุด โดยเฉพาะเมื่อสั่งการผ่าน Script หรือระบบอัตโนมัติ (Agentic Workflow) กรุณาทำตามกฎเหล่านี้:

1. **ห้ามใช้ `--interactive` หากส่งคำสั่งผ่าน Script/Pipe:** 
   การส่ง Input ทีละบรรทัดเข้าไปในโหมด Interactive จะทำให้ Standard Input (stdin) ถูกปิดเมื่อสิ้นสุดคำสั่ง (EOF) ส่งผลให้เบราว์เซอร์ปิดตัวเองกะทันหันและเกิด Error `Target page, context or browser has been closed`
2. **วิธีรันที่ดีที่สุดคือ One-liner (จบในบรรทัดเดียวพร้อมโหมด Headless):**
   ให้ระบุ Prompt ลงไปในพารามิเตอร์ตรงๆ และตั้งค่า Session Directory ให้ถูกต้อง:
   ```powershell
   $env:DS_SESSION_DIR="C:\Users\BUSOLOVE\.deepseek-agent\session_1"
   dsa --headless "คำสั่งที่ต้องการให้ทำ..."
   ```
3. **การควบคุม Format สำหรับ Tool Calls:**
   เมื่อต้องการให้ `ds` เรียกใช้ Tools (เช่น `run_command`) ต้องกำชับให้ `ds` ตอบกลับเป็น JSON code block เสมอ (ห้ามใช้ XML tags) เพื่อให้ `parser.js` ภายในระบบอ่านค่าได้สำเร็จ:
   ` ```json {"name": "run_command", "args": {"command": "..."}} ``` `
4. **วิธีแก้ปัญหาบอทหาช่องแชทไม่เจอ (Cannot find the DeepSeek chat input box):**
   หาก DeepSeek อัปเดตหน้าเว็บ (UI Changed) ให้รันโหมด Calibration เพื่อเรียนรู้ DOM Selectors ใหม่:
   ```powershell
   $env:DS_SESSION_DIR="C:\Users\BUSOLOVE\.deepseek-agent\session_1"; dsa --calibrate
   ```
