# SPEC: COMPUTATION FLOW — ส่งข้อมูลผ่าน RAM 100% ประมวลผลขนานอย่างรวดเร็ว

## หลักการประมวลผล (RAM-First Architecture)
ระบบปรับปรุงสถาปัตยกรรมสู่ **RAM 100% In-Memory Data Passing** เพื่อลดเวลา latency จาก 20 วินาที เหลือเพียง < 1.5 วินาที สำหรับ 4 สินทรัพย์

1. **ส่วนงานแรก (Data Feed / DataAdapter):** ดึงข้อมูลราคา Real-time ผ่าน WebSocket เก็บไว้ใน RAM (`_completed_candles`) ไม่มีการเขียน CSV ลง Disk ในระหว่างรอบการวิเคราะห์ (เขียน CSV เฉพาะตอนเริ่มระบบ `init_symbol` เท่านั้น)
2. **ส่วนงานที่สอง (Analysis / Orchestrator):** รับ `candles_dict` (M1, M5, M15) จาก RAM โดยตรงผ่าน `runner.py` เข้าประมวลผล Parallel Engines และส่งออกไฟล์ผลลัพธ์ 78 ฟิลด์ (TXT) ไปยัง `data_base/orchestrator/<SYMBOL>/<PROMPT_ID>.txt`
3. **ส่วนงานที่สาม (Execution / AI Layer):** อ่านไฟล์ผลลัพธ์ 78 ฟิลด์เพื่อวิเคราะห์และส่งคำสั่งเทรดต่อ
...
```
[ส่วนงานแรก: Data Feed & RAM Cache]
ดึงราคาจาก IQ Option WebSocket Stream
      ↓
อัปเดตและเก็บแท่งเทียนสมบูรณ์ใน RAM (_completed_candles)
(ไม่มี Disk CSV I/O ในระหว่าง cycle)
      ↓
======================= สิ้นสุดส่วนงานแรก =======================
      ↓
[ส่วนงานที่สอง: Analysis / Orchestrator (RAM 100%)]
runner.py ดึง candles_dict จาก RAM ส่งตรงให้ orchestrator.process_cycle()
      ↓
ประมวลผลคำนวณทางเทคนิค (Indicator Store) + เครื่องมือชั้นสูง (Advanced Tools)
      ↓
ส่งวิเคราะห์ Tier-1 Engines ทั้ง 5 ชุดแบบขนาน (ThreadPoolExecutor Parallel Processing)
      ↓
วิเคราะห์จำแนกสถานะตลาด (Market State Classifier) และจัดระดับความเสี่ยง
      ↓
ส่งวิเคราะห์ 10 Supplementary Engines ขนานกัน
      ↓
บันทึกแบบจำลองข้อมูล 78 ฟิลด์ ลงไฟล์ .txt
