...
| **Pivot Points** | `StructuralIndicators` | `pivot`, `r1`, `r2`, `s1`, `s2` <br>• **วิธีคำนวณ:** ใช้แท่งเทียน M5 ที่ปิดสมบูรณ์เท่านั้น (ไม่ใช่แท่งก่อนตัว) <br>• **สูตร:** Pivot = (High + Low + Close) / 3 ของแท่งที่ปิดสมบูรณ์ <br>• **การเลือกแท่ง:** ใช้แท่งล่าสุด (`iloc[-1]`) เป็นแท่งเทียนที่ปิดสมบูรณ์เสมอ เนื่องจาก `data_feed` (Part 1) ตัดแท่งที่ยังไม่ปิด (forming candle) ออกไปแล้วก่อนเขียนลง CSV/RAM (ดู `data_adapter.py`: `drop_forming()` ใน `init_symbol()`/`update()`) — แถวสุดท้ายของ CSV จึงเป็นแท่งที่ปิดสมบูรณ์เสมอ ไม่ต้องถอยไปใช้ `iloc[-2]` <br>• **หมายเหตุ (M1 ก็ใช้หลักการเดียวกัน):** M1 pivot ก็ใช้ `iloc[-1]` ด้วยเหตุผลเดียวกันนี้ ทั้ง M1 และ M5 จึงสอดคล้องกัน |
...
| **Pivot Points** | `StructuralIndicators` | `pivot`, `r1`, `s1` — ใช้แท่งล่าสุด (`iloc[-1]`) เสมอ เหตุผลเดียวกับ M5 ด้านบน |
...
4. **Warm-up ขั้นต่ำ:** ต้องมีแท่งเทียนอย่างน้อย 150 แท่งในทุก timeframe (M1, M5, M15) ก่อนเริ่มคำนวณ (`FAIL-FAST` หากไม่ครบ) — เผื่อ buffer จากค่าต่ำสุดจริงที่ 100 แท่ง (ซึ่ง `bbw_sma_100` ทุก timeframe ต้องการ)
