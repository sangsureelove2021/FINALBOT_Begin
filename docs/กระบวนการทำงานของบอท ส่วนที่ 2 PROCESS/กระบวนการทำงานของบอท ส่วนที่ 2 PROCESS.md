...
flowchart TD
    ...
    G -->|Merge 78 Fields| H[Output: data_base/orchestrator/...]
...
├── orchestrator.py            → ศูนย์กลางควบคุมการรับส่งข้อมูลผ่าน RAM และจัดการ Data Flow (👑)
...
1. รับ `candles_dict` (M1, M5, M15) จาก RAM โดยตรงผ่าน `runner.py` (ไร้ Disk CSV I/O)
...
ระบบรวบรวมข้อมูลดิบ + ผลวิเคราะห์จากทุก Engine จำนวน **78 ฟิลด์** จัดรูปแบบในโครงสร้าง YAML ลงไฟล์นามสกุล `.txt` 
- บันทึกไว้ที่: `data_base/orchestrator/<SYMBOL>/[filename].txt`
