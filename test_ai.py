import sys
sys.path.insert(0, r"E:\BOT_FINALBOT13 STG\BOT_FINALBOT")
sys.stdout.reconfigure(encoding='utf-8')
import logging
from core.ai_analysis.deepseek_agent_bridge import DeepSeekAgentBridge
from core.ai_analysis.prompt_ai_context import build_prompt

logging.basicConfig(level=logging.INFO)

yaml_payload = """meta:
  timestamp: 2026-06-26T09:11:02.493221
  symbol: EURGBP
  session: ASIAN
  price: 0.86129
  data_age_ms: 62606
  data_quality: HIGH

market_context:
  state: SIDEWAY_RANGE
  description: Price ranging between clear levels. Suitable for mean-reversion strategies.
  volatility_regime: NORMAL
  news_impact: LOW
  expected_volatility_%: 0.012

timeframes:
  m1:
    last_candle: 0.86129
    ema5: 0.861278
    ema20: 0.861243
    rsi: 65.08
    stoch_k: 79.71
    stoch_d: 79.95
    macd: 0.000019
    macd_signal: 0.00001

  m5:
    bias: BEARISH
    ema5: 0.861242
    ema10: 0.861248
    ema20: 0.86131
    ema50: 0.86146
    bb_upper: 0.861608
    bb_lower: 0.86104
    bb_width: 0.000569
    rsi: 34.62
    stoch_k: 36.38
    stoch_d: 27.95
    macd: -0.00009
    macd_signal: -0.000101
    adx: 38.79
    atr: 0.000103
    support: 0.861095
    resistance: 0.861595
    pivot: 0.861283

  m15:
    bias: BEARISH

price_action:
  pattern: NONE
  last_candle_bias: BEARISH
  body_strength: WEAK
  wick_dominance: HIGH
  momentum_bias: BEARISH
  move_quality: CHOPPY
  trap_alert: true
  sr_interaction: NONE

volume:
  tick_volume: 524
  volume_momentum: FALLING
  volume_vs_average: 0.704

analysis:
  trend_direction: DOWN
  trend_type: CHOPPY
  trend_strength_score: 20
  mtf_alignment_%: 50
  compression_quality_%: 42.37904424778761
  exhaustion_risk_%: 40
  bos_detected: false

signals:
  triggered: 
  count: 0
  top_signal: NO

decision_layer:
  tradeable: true
  tradeable_reason: Passed basic checks
  confidence_score: 0
  stability_score: 50
  quality_score: 50
  risk_level: MEDIUM
  suggested_expiry_minutes: 5
  suggested_action: WAIT
  final_reason_th: รอการวิเคราะห์จาก AI
"""

context = {
    "text_prompt": yaml_payload,
    "symbol": "EURGBP",
    "timeframes": {
        "m5": {
            "rsi": 34.62,
            "ema5": 0.861242
        }
    }
}

bridge = DeepSeekAgentBridge()
print("1. Checking Readiness...")
print(bridge.check_readiness())
print("-" * 50)
print("2. Sending Payload to AI...")
insight = bridge.analyze_market(context)

print("\n=== AI RESPONSE PARSED ===")
if insight:
    print(f"Action:     {insight.action}")
    print(f"Confidence: {insight.confidence}%")
    print(f"Expiry:     {insight.expiry} minutes")
    print(f"Reason:     {insight.reason}")
    print("\n--- RAW TEXT FROM AI ---")
    print(insight.raw_response)
else:
    print("AI returned None or failed to parse.")
