...
class BehaviorAnalyzer(BaseEngine):
    ...
    def _conviction(self, df: pd.DataFrame) -> int:
        ...
        rng = df['high'] - df['low']
        if (rng == 0).any():
            raise ValueError("Division by zero in conviction calculation (rng == 0)")
    ...
    def _hesitation(self, df: pd.DataFrame) -> int:
        ...
        rng = df['high'] - df['low']
        if (rng == 0).any():
            raise ValueError("Division by zero in hesitation calculation (rng == 0)")
        body = (df['close'] - df['open']).abs()
        ratio = body / rng
    ...
    def _pressure_balance(self, df: pd.DataFrame) -> float:
        ...
        total = bull + bear
        if total == 0:
            raise ValueError("Division by zero: Total pressure is 0")
