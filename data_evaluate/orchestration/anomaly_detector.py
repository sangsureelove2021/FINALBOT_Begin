...
class AnomalyDetector:
    ...
    def detect_price_spikes(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        # Calculate rolling volatility based on ohlcv-processing skill
        spike_window = self.anomaly_config.get("spike_window", 20)
        spike_threshold = self.anomaly_config.get("spike_threshold", 0.3)
        rolling_std = df['price_change'].rolling(window=spike_window, min_periods=5).std()
        
        # Detect spikes (price change > threshold * rolling std or absolute threshold)
        std_spikes = df['price_change'].abs() > (spike_threshold * rolling_std)
        if spike_threshold < 1.0:
            abs_spikes = df['price_change'].abs() > spike_threshold
            df['spike_detected'] = (std_spikes | abs_spikes).fillna(False)
        else:
            df['spike_detected'] = std_spikes.fillna(False)
        
        # Count spikes
        spike_count = int(df['spike_detected'].sum())
        if spike_count > 0:
    ...
    def detect_zero_volume(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        if 'volume' not in df.columns:
            logger.warning(f"[ANOMALY] Volume column missing for {symbol}")
            df['zero_volume_detected'] = False
            df['zero_volume_streak'] = 0
            return df
        
        threshold = self.anomaly_config.get("zero_volume_threshold", 0)
        df['zero_volume_detected'] = df['volume'] <= threshold
        
        # Count consecutive zero volumes
        zero_mask = df['zero_volume_detected']
        df['zero_volume_streak'] = zero_mask.groupby((~zero_mask).cumsum()).cumsum()
        
        # Count zero volumes
        zero_vol_count = int(df['zero_volume_detected'].sum())
        if zero_vol_count > 0:
            self.counters["zero_volume_count"] += zero_vol_count
            
            # Check for consecutive violations
            max_streak = int(df['zero_volume_streak'].max()) if not df['zero_volume_streak'].empty else 0
    ...
    def detect_impossible_candles(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        # Detect impossible candles per ohlcv-processing standards
        ...
        if 'volume' in df.columns:
            impossible_conditions.append(df['volume'] < 0)  # Negative volume
        ...
        impossible_count = int(df['impossible_candle_detected'].sum())
    ...
    def detect_anomalies(self, df: pd.DataFrame, symbol: str, response_time: Optional[float] = None) -> pd.DataFrame:
        ...
        if existing_columns:
            ...
            total_anomalies = int(result_df['any_anomaly'].sum())
    ...
    def _log_anomalies(self, anomaly_df: pd.DataFrame, symbol: str, anomaly_type: str) -> None:
        ...
        try:
            ...
            logger.exception(f"[ANOMALY] Failed to log anomalies for {symbol}: {e}")
