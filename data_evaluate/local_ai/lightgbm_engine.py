...
- Extracts Multi-Timeframe Price Action & 96 Indicators from RAM (< 2 ms)
...
- 100% In-Memory, Type-Strict, Defensive Fail-Fast
"""
...
MODEL_FILE = os.path.join(MODEL_DIR, "lightgbm_binary_model.joblib")
...
class LightGBMEngine:
    ...
    def _build_calibrated_model(self):
        ...
        try:
            ...
            X_cal, y_cal = self._generate_calibration_dataset()
            clf.fit(X_cal, y_cal)
            self.model = clf
    ...
    @staticmethod
    def _generate_calibration_dataset() -> Tuple[np.ndarray, np.ndarray]:
        """
        Generates structured synthetic market regime features:
        Classes: 0 = WAIT, 1 = CALL, 2 = PUT
        Features (12 dims):
        [0] m5_body_ratio, [1] m5_upper_wick_ratio, [2] m5_lower_wick_ratio, [3] m5_rsi_14,
        [4] m5_stoch_k, [5] m5_bb_pct_b, [6] m15_trend_slope, [7] m5_trend_slope,
        [8] mtf_alignment, [9] rejection_detected, [10] room_to_target, [11] volatility_atr_ratio
        """
        np.random.seed(42)
        samples = 3000
        X = np.random.uniform(0.0, 1.0, size=(samples, 12)).astype(np.float64)
        y = np.zeros(samples, dtype=int)

        X[:, 3] = np.random.uniform(10, 90, size=samples)
        X[:, 4] = np.random.uniform(5, 95, size=samples)
        X[:, 6] = np.random.uniform(-1.0, 1.0, size=samples)
        X[:, 7] = np.random.uniform(-1.0, 1.0, size=samples)
        X[:, 8] = np.random.choice([-1.0, 0.0, 1.0], size=samples)
        X[:, 9] = np.random.choice([-1.0, 0.0, 1.0], size=samples)

        for i in range(samples):
            rsi, stoch, bb_pct, mtf, rej, room, lower_wick, upper_wick = (
                X[i, 3], X[i, 4], X[i, 5], X[i, 8], X[i, 9], X[i, 10], X[i, 2], X[i, 1]
            )
            if (rsi < 35 or stoch < 25 or bb_pct < 0.15) and (rej == 1.0 or lower_wick > 0.4) and room > 0.35 and mtf >= 0:
                y[i] = 1 # CALL
            elif (rsi > 65 or stoch > 75 or bb_pct > 0.85) and (rej == -1.0 or upper_wick > 0.4) and room > 0.35 and mtf <= 0:
                y[i] = 2 # PUT
            else:
                y[i] = 0 # WAIT
    ...
    def evaluate(self, symbol: str, candles: Optional[Dict[str, pd.DataFrame]] = None, payload_dict: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Executes Model B (LightGBM) evaluation.
        """
        result = {
            "symbol": symbol,
