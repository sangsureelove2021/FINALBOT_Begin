...
output_path = r"C:\Users\Administrator\.gemini\antigravity\brain\bf808087-316a-4062-af9c-a51da648c8ed\scratch\proposed_arbitration.txt"
    ...
    if strategy == "compression_breakout":
        if orig_direction == "CALL":
            if ema5 > ema20 and ema20 > ema50:
                if rsi7 < 75:
                    decision = "CALL"
                    detail = f"Keep CALL: Strong uptrend (EMA5={ema5:.5f} > EMA20={ema20:.5f} > EMA50={ema50:.5f}) and RSI7={rsi7:.1f} < 75."
                else:
                    detail = f"Change to NOTRADE: Overbought RSI7={rsi7:.1f} >= 75."
            else:
            ...
            if ema5 < ema20 and ema20 < ema50:
                if rsi7 > 25:
                    decision = "PUT"
                    detail = f"Keep PUT: Strong downtrend (EMA5={ema5:.5f} < EMA20={ema20:.5f} < EMA50={ema50:.5f}) and RSI7={rsi7:.1f} > 25."
                else:
                    detail = f"Change to NOTRADE: Oversold RSI7={rsi7:.1f} <= 25."
            else:
                detail = f"Change to NOTRADE: Trend not established (EMA5={ema5:.5f}, EMA20={ema20:.5f}, EMA50={ema50:.5f})."
                
    elif strategy == "stochastic_crossover":
        if orig_direction == "CALL":
            # Crossover check: K crossed above D in oversold zone (< 25)
            ...
            # Crossover check: K crossed below D in overbought zone (> 75)
            is_crossover = prev_stoch_k >= prev_stoch_d and stoch_k < stoch_d
        ...
        # Check close price from reason
        # e.g., "America EURUSD BB+RSI triggered CALL: Close 1.13021 <= Lower BB 1.13057"
        close_match = re.search(r"Close\s+(\d+\.\d+)", reason)
        close_val = float(close_match.group(1)) if close_match else None
        
        if orig_direction == "CALL":
            # Close <= bb_lower and rsi7 < 35
            is_oversold_rsi = rsi7 < 35
            is_at_bb = (close_val <= bb_lower) if close_val else (ema5 <= bb_lower * 1.0005) # fallback
            ...
            # Close >= bb_upper and rsi7 > 65
            is_overbought_rsi = rsi7 > 65
            is_at_bb = (close_val >= bb_upper) if close_val else (ema5 >= bb_upper * 0.9995) # fallback
...
print(f"Proposed decisions saved to proposed_arbitration.txt")
