...
def main():
    ...
    for i in range(start_index, end_index + 10):
        ...
        for order_id, trade in active_trades:
            ...
            expiry_minutes = 2 if trade.expiry == 'M2' else 1
                ...
                try:
                    ...
                    if action in ('CALL', 'PUT') or res.get('entry_score', 0.0) > 0.0 or res.get('block_score', 0.0) > 0.0:
                        ...
                        if action in ('CALL', 'PUT') and res.get('block_score', 0.0) < 100.0 and res.get('entry_score', 0.0) >= 51.0 and not bot.order_manager.get_active_trades(symbol):
                            ...
                            # --- CHANGED: Shoot 2 orders (M1 and M2) ---
                            from execution.order_manager import Trade
                            
                            # Order M1
                            order_id_1m = str(uuid.uuid4())[:8] + "_1m"
                            trade_1m = Trade(
                                order_id=order_id_1m,
                                symbol=symbol,
                                direction=action,
                                amount=35.0,
                            ...
                            bot.order_manager.active_trades[order_id_1m] = trade_1m
                            ...
                            # Order M2
                            order_id_2m = str(uuid.uuid4())[:8] + "_2m"
                            trade_2m = Trade(
                                order_id=order_id_2m,
                                symbol=symbol,
                                direction=action,
                                amount=35.0,
                                entry_price=context.current_price,
                            ...
                            bot.order_manager.active_trades[order_id_2m] = trade_2m
                            ...
                            print(f"   🚀 Signal: {strategy.strategy_name} -> {action} @ {context.current_price:.5f} | Score: {res.get('entry_score', 0.0)} | FIRED BOTH 1M AND 2M ORDERS!")
