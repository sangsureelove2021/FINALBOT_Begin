...
def run_simulation(use_snr):
    strategy_stats = {}
    payout_rate = 0.85
    stake = 35.0
    
    df_m5['touched_support_3c'] = df_m5['min_low_3c'] <= df_m5['local_support_3c'] * 1.0002 if use_snr else True
    df_m5['touched_resistance_3c'] = df_m5['max_high_3c'] >= df_m5['local_resistance_3c'] * 0.9998 if use_snr else True
    ...
    for w_idx, (sat_yr, sat_mo, sat_dy, sun_yr, sun_mo, sun_dy) in enumerate(WEEKENDS):
        ...
        for i in range(len(df_week) - 1):
            ...
            if triggered_strategy is not None:
                ...
                if triggered_strategy not in strategy_stats:
                    strategy_stats[triggered_strategy] = {'trades': 0, 'wins': 0, 'losses': 0, 'pnl': 0.0}
                    
                stats = strategy_stats[triggered_strategy]
                stats['trades'] += 1
                if won:
                    stats['wins'] += 1
                    stats['pnl'] += stake * payout_rate
                else:
                    stats['losses'] += 1
                    stats['pnl'] -= stake
...
before_stats = run_simulation(use_snr=False)
after_stats = run_simulation(use_snr=True)

print("\nCOMPARISON TABLE:")
all_strategies = set(before_stats.keys()).union(set(after_stats.keys()))

for s in sorted(all_strategies):
    b = before_stats.get(s, {'trades': 0, 'wins': 0, 'losses': 0, 'pnl': 0.0})
    a = after_stats.get(s, {'trades': 0, 'wins': 0, 'losses': 0, 'pnl': 0.0})
    
    b_wr = (b['wins'] / b['trades'] * 100) if b['trades'] > 0 else 0.0
    a_wr = (a['wins'] / a['trades'] * 100) if a['trades'] > 0 else 0.0
    
    pnl_diff = a['pnl'] - b['pnl']
    
    print(f"Strategy: {s}")
    print(f"  - BEFORE: Trades={b['trades']:<4} | Wins={b['wins']:<4} | Losses={b['losses']:<4} | WinRate={b_wr:.2f}% | PnL={b['pnl']:+.2f} THB")
    print(f"  - AFTER : Trades={a['trades']:<4} | Wins={a['wins']:<4} | Losses={a['losses']:<4} | WinRate={a_wr:.2f}% | PnL={a['pnl']:+.2f} THB")
    print(f"  - CHANGE: Trades={a['trades']-b['trades']:+d} | WinRate={a_wr-b_wr:+.2f}% | PnL={pnl_diff:+.2f} THB")
