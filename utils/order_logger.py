"""
Standardized order logging for live trades.
...
def log_trade_outcome(order_id: str, exit_price: float, pnl: float, outcome: str, notes: str = "") -> None:
    """
    Helper to update an existing trade with outcome. Since JSONL is append-only,
    this logs a separate outcome record with the same order_id.
    """
    record = {
