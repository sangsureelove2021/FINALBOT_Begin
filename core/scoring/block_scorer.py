...
class BlockScorer:
    ...
    def score(self, context: MarketContext) -> int:
        ...
        if not isinstance(context, MarketContext):
            logger.error(f"Invalid context type passed to BlockScorer.score: {type(context)}")
            return 0
    ...
    def get_block_reasons(self, context: MarketContext) -> list:
        ...
        if not isinstance(context, MarketContext):
            logger.error(f"Invalid context type passed to BlockScorer.get_block_reasons: {type(context)}")
            return []
