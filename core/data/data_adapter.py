...
class DataAdapter:
    ...
    def update(
        ...
        try:
            ...
            store_m1_df = self._store_m1.get(symbol)
            if not isinstance(store_m1_df, pd.DataFrame) or store_m1_df.empty or 'close' not in store_m1_df.columns:
                logger.error(f"[DataAdapter] M1 store for {symbol} is empty, not a DataFrame, or missing 'close' column")
                return None
            ...
            logger.exception(f"[DataAdapter] update({symbol}) failed")
            return None
    ...
    def _refresh_m5(
        ...
        if not isinstance(current_min, int):
            raise TypeError("current_min must be an integer")
        if not isinstance(prefix, str):
            raise TypeError("prefix must be a string")
    ...
    def _merge(
        ...
        if stored is None or stored.empty:
            logger.warning(f"[DataAdapter] {label}: stored DataFrame is empty or invalid in _merge")
            if fresh is not None and not fresh.empty:
                return fresh.tail(200).copy()
            return pd.DataFrame()
        ...
        if gap_sec > gap_threshold:
            ...
            if full is not None and not full.empty:
                return full
            return stored.copy()  # keep old data if refetch fails
    ...
    def _write_csv(df: Optional[pd.DataFrame], directory: str, filename: str) -> None:
        ...
        try:
            ...
            logger.exception(f"[DataAdapter] CSV write failed ({filename})")
