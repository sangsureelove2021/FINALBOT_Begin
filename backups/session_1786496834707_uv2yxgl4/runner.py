...
class PureAIRunner:
    ...
    def run_cycle(self):
        ...
        if current_minute != self.last_displayed_minute:
            ...
            for sym in self.symbols:
                try:
                    payload = self.orchestrator.process_cycle(symbol=sym)
                    if payload:
                        logger.info(f"[Runner] Part 2 payload generated for {sym}")
                except Exception as e:
                    logger.exception(f"[Runner] Part 2 failed for {sym}: {e}")
