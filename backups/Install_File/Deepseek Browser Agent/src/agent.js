...
class DeepSeekAgent {
  ...
  async run(task) {
    ...
    for (let iter = 1; iter <= maxIter; iter++) {
      ...
      // ── Case 1: Tool call ─────────────────────────────────────────────
      if (this._shouldUseTools(parsed)) {
  ...
  // ── Tool Usage Rules ──────────────────────────────────────────────────
  /**
   * Check if tools should be used for this task based on strict rules
   */
  _shouldUseTools(parsed) {
    // AI ตัดสินใจเองจาก System Prompt ว่าจะส่ง tool_call มาเมื่อไหร่
    // ถ้า AI ส่ง tool_call มา = AI ต้องการใช้เครื่องมือจริง → ให้ใช้ได้เลย
