...
const TOOLS = {
  ...
  // ── Search in Files (Windows-compatible) ───────────────────────────────────────
  search_in_files: {
    description: 'Search for text patterns inside files (Windows findstr). Returns matching lines with filenames.',
    parameters: {
    ...
    async execute({ pattern, directory = '.', file_pattern, case_sensitive = false, context_lines = 2 }) {
      ...
      // For Windows, use findstr command which is more reliable
      const escapedDir = `"${dir}"`;
      const escapedPattern = case_sensitive ? `"${pattern}"` : `/I "${pattern}"`;
      
      // Build file pattern filter
      let fileFilter = '';
      if (file_pattern) {
        const extensions = file_pattern.split(',').map(ext => {
          const cleanExt = ext.trim().replace(/^\./, '');
          return `*.${cleanExt}`;
        }).join(' ');
        fileFilter = ` /M "${extensions}"`;
      }
      
      // Use findstr with better Windows compatibility
      const cmd = `findstr /S /N ${escapedPattern}${fileFilter} ${escapedDir} 2>nul`;
      ...
      try {
        ...
        // Process results to add context and formatting
        const lines = result.split('\n').filter(line => line.trim());
        const formattedResults = [];
        
        for (const line of lines.slice(0, 150)) { // Limit results
          ...
          if (match) {
            ...
            if (context_lines > 0) {
              ...
              formattedResults.push(`[${relativePath}:${lineNumber}]`);
              formattedResults.push(`>>> ${lineNumber}: ${content.trim()}`);
            } else {
              formattedResults.push(`[${relativePath}:${lineNumber}] ${content.trim()}`);
            }
          }
        }
        
        return truncate(formattedResults.join('\n').trim());
      } catch (err) {
        if (err.status === 1 || err.signal === 'SIGTERM') {
          return `No matches found for: ${pattern}`;
        }
        throw err;
      }
