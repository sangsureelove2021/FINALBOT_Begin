...
class IQOptionAdapter(IDataSource):
    ...
    def _get_cached_candles(self, symbol: str, timeframe: str) -> pd.DataFrame:
        """Get candles from WebSocket memory cache in a thread-safe manner."""
        with self._cache_lock:
            try:
                if not self.api:
                    logger.warning("[Cache] API not initialized")
                    return pd.DataFrame()
                
                size = self._TF_SECONDS.get(timeframe)
                if not size:
                    logger.warning(f"[Cache] Unsupported timeframe: {timeframe}")
                    return pd.DataFrame()
                
                # Get cached data from WebSocket API
                ...
                if not cached_data:
                    logger.debug(f"[Cache] No cached data for {symbol} {timeframe}")
                    return pd.DataFrame()
                
                if isinstance(cached_data, dict):
                    df = pd.DataFrame.from_dict(cached_data, orient='index')
                    if 'from' not in df.columns:
                        df = df.reset_index().rename(columns={'index': 'from'})
                ...
                if not all(col in df.columns for col in required_cols):
                    logger.warning(f"[Cache] Missing required columns in cached data for {symbol}: {required_cols}")
                    return pd.DataFrame()
                
                if "from" in df.columns:
                    df["timestamp"] = pd.to_datetime(pd.to_numeric(df["from"], errors='coerce'), unit="s", utc=True)
                ...
                logger.warning(f"[Cache] Failed to get cached data for {symbol} {timeframe}: {e}")
                return pd.DataFrame()
    ...
    def update_with_streaming(self, symbol: str, timeframe: str, count: int) -> pd.DataFrame:
        ...
        First tries WebSocket RAM Cache; if empty or disconnected, gracefully falls back to REST API.
        ...
        Returns:
            DataFrame with updated candles indexed by datetime
        """
        total_start = time.time()
        
        try:
            # 1. Start/verify stream setup
            self._start_realtime_stream(symbol, timeframe, count)
            
            # 2. Try fetching from thread-safe WebSocket RAM cache
            df_cache = self._get_cached_candles(symbol, timeframe)
            
            if df_cache is not None and not df_cache.empty and len(df_cache) >= 2:
                total_time = time.time() - total_start
                logger.info(f"[Streaming] Retriving {symbol} {timeframe} ({len(df_cache)} candles) from WebSocket Memory Cache in {total_time*1000:.2f}ms")
                return df_cache.tail(count)
            else:
                logger.warning(f"[Streaming] WebSocket Cache empty/insufficient for {symbol} {timeframe}. Graceful Fallback to REST API.")
        except Exception as e:
            logger.warning(f"[Streaming] Streaming cache error for {symbol} {timeframe}: {e}. Graceful Fallback to REST API.")
            
        # 3. Graceful Fallback to REST API
        logger.info(f"[Streaming] Graceful Fallback -> Fetching via REST API for {symbol} {timeframe}")
