...
class DataAdapter(IDataSource):
    ...
    def _merge(self, stored: Optional[pd.DataFrame], fresh: Optional[pd.DataFrame],
        ...
        """Merge stored and fresh data with gap detection and price continuity validation."""
        ...
        # --- CRITICAL: Validate price continuity to detect symbol mixing ---
        # Check if close prices from stored and fresh are in the same price range
        # This catches cases where data from different symbols gets mixed
        stored_close_median = float(stored['close'].median()) if 'close' in stored.columns else None
        fresh_close_median = float(fresh['close'].median()) if 'close' in fresh.columns else None
        ...
        if stored_close_median is not None and fresh_close_median is not None:
            ...
            # If price difference is more than 10%, this is likely symbol mixing
            # Different currency pairs have very different price levels
            if price_diff_ratio > 0.10:  # 10% threshold
        ...
        if len(overlap) > 1 and 'close' in stored.columns and 'close' in fresh.columns:
            check = overlap[:-1][-4:]
            if len(check) > 0:
                # Additional check: ensure overlapping candles are consistent with symbol
                # If they differ significantly, this indicates data corruption
                stored_close_values = stored.loc[check, 'close']
                fresh_close_values = fresh.loc[check, 'close']
                ...
                if overlap_median_rel_diff > 0.05:  # 5% difference in overlap
