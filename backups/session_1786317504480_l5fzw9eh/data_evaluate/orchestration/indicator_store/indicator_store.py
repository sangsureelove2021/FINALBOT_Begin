...
class IndicatorStore:
    ...
    def calculate_raw_indicators(df_m1: pd.DataFrame, df_m5: pd.DataFrame, df_m15: Optional[pd.DataFrame] = None, forming_data: Optional[Dict[str, Any]] = None, symbol: Optional[str] = None) -> Dict[str, Any]:
        ...
        if df_m15 is not None and not df_m15.empty:
            ...
            # M15 data staleness check:
            # OTC pairs trade 24/7 (allow up to 8 hours), Non-OTC pairs close on weekends (allow up to 72 hours)
            is_otc_sym = True if (symbol and "OTC" in symbol.upper()) else False
            max_allowed_age = 28800000 if is_otc_sym else 259200000  # 8h for OTC, 72h for Non-OTC (weekend)
            if m15_age_ms > max_allowed_age:
                raise ValueError(f"FAIL-FAST: M15 data is STALE for {symbol} (age: {m15_age_ms} ms). No fallback allowed.")
