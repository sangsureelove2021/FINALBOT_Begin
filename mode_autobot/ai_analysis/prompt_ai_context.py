    ...
    if "timeframes" in context and isinstance(context.get("timeframes"), dict):
        normalized["timeframes"] = {
            "m1": {
                ...
                "ohclv": {
                    "open": _get_first_present([context["timeframes"].get("m1", {}).get("ohclv", {}).get("open")], "NOT_AVAILABLE"),
                    "high": _get_first_present([context["timeframes"].get("m1", {}).get("ohclv", {}).get("high")], "NOT_AVAILABLE"),
                    "low": _get_first_present([context["timeframes"].get("m1", {}).get("ohclv", {}).get("low")], "NOT_AVAILABLE"),
                    "close": _get_first_present([context["timeframes"].get("m1", {}).get("ohclv", {}).get("close")], "NOT_AVAILABLE"),
                    "volume": _get_first_present([context["timeframes"].get("m1", {}).get("ohclv", {}).get("volume")], "NOT_AVAILABLE"),
                }
            },
            "m5": {
                ...
                "ohclv": {
                    "open": _get_first_present([context["timeframes"].get("m5", {}).get("ohclv", {}).get("open")], "NOT_AVAILABLE"),
                    "high": _get_first_present([context["timeframes"].get("m5", {}).get("ohclv", {}).get("high")], "NOT_AVAILABLE"),
                    "low": _get_first_present([context["timeframes"].get("m5", {}).get("ohclv", {}).get("low")], "NOT_AVAILABLE"),
                    "close": _get_first_present([context["timeframes"].get("m5", {}).get("ohclv", {}).get("close")], "NOT_AVAILABLE"),
                    "volume": _get_first_present([context["timeframes"].get("m5", {}).get("ohclv", {}).get("volume")], "NOT_AVAILABLE"),
                }
            },
            "m15": {
...
HEADER = """คุณคือผู้เชี่ยวชาญด้านการวิเคราะห์ตลาด Binary Options ที่มีประสบการณ์มากกว่า 10 ปี 
คุณเชี่ยวชาญด้านการวิเคราะห์เชิงเทคนิคอย่างลึกซึ้ง ครอบคลุมทุกมิติของตลาด
คำสั่ง: ให้คุณอ่านข้อมูลทุกฟิลด์ที่ส่งให้ ID:{prompt_id} แล้วนำไปวิเคราะห์หาสัญญาณซื้อขาย Binary Option 

"""

FOOTER_TEMPLATE = """
กฎเด็ดขาด-อ่านและวิเคราะห์ข้อมูลราคาตลาดในข้อความ ID:{prompt_id} แล้วตอบกลับ output เฉพาะ JSON ตามตัวอย่างนี้เท่านั้น:

{{"action":"CALL","confidence":99,"expiry":5,"reason":"บอกเหตุผลเป็นภาษาไทย พร้อมจุดราคาที่ควรเข้าเทรดหากสัญญาณคือ CALL PUT ไม่เกิน 30 คำ"}}

กฎ output:
- action: "CALL" | "PUT" | "NO_TRADE"
- confidence: ตัวเลขจำนวนเต็ม 0-100
- expiry: ตัวเลขจำนวนเต็ม 1-5 นาที
- reason: บอกเหตุผลเป็นภาษาไทย พร้อมจุดราคาที่ควรเข้าเทรดหากสัญญาณคือ CALL PUT ไม่เกิน 30 คำ
...
- OUTPUT เฉพาะรูปแบบ JSON OBJECT เท่านั้น
- ต้องตอบกลับภายใน 15 วินาที หลังจากได้รับข้อมูลนี้
"""
...
def save_prompt_to_disk_raw(prompt_id: str, symbol_str: str, prompt: str) -> str:
    ...
    history_folder = project_root / "all_filelogs" / "logs_ai" / symbol_str
...
def build_prompt(filepath: str) -> str:
    ...
    try:
        ...
        header_formatted = HEADER.format(prompt_id=prompt_id)
        footer_formatted = FOOTER_TEMPLATE.format(prompt_id=prompt_id)
        final_prompt = f"{header_formatted}{yaml_content}\n{footer_formatted}"
