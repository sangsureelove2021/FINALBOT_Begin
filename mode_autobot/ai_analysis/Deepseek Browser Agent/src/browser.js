...
// Performance optimization: Reduce timeouts
const TIMEOUTS = {
  PAGE_LOAD: 10000,      // Reduced from 30000
  ELEMENT_WAIT: 3000,    // Reduced from 10000
  RESPONSE_WAIT: 15000,   // Reduced from 30000
  NAVIGATION: 8000       // Reduced from 20000
};
...
class DeepSeekBrowser {
  ...
  async launch() {
    // Show launch message with icon
    if (process.env.DS_QUIET !== 'true') {
      console.log(' ℹ  Launching browser with persistent session...');
    }

    const sessionDir = path.resolve(config.SESSION_DIR);

    // Performance optimization: Reduce browser startup time
    const launchOptions = {
      ...
      javaScriptEnabled: true,                      // Enable JS for faster processing
      acceptDownloads: false,                       // Disable downloads
    };
    ...
    // Only show browser ready message when not in quiet mode
    if (process.env.DS_QUIET !== 'true') {
  ...
  async newChat() {
    ...
    await this._navigate(config.DEEPSEEK_URL);
    // logger.dim('Navigated to DeepSeek home (new chat)'); // Commented out to reduce duplicate messages
  }
